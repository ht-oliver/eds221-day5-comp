...
# Creating a time series of years
time_yr <- seq(from = 0, to = 8, by = 1)

# Creating a matrix that is the number of ages and years of study. E, F, J, A by Years. 
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# Populating first row of matrix with Year 0 data
proj_year[1, ] <- fish_year0

# Fill in the proj_year matrix with the product of the Leslie matrix and the previous year of data in the proj_year matrix.
for (i in 2:length(time_yr)) { # Using 2:length because we already filled in the first row
...
proj_df <- data.frame(time_yr, proj_year) %>% # Pipe the data frame down to your pivot operation
  pivot_longer(cols = -time_yr, names_to = "lifestage", values_to = "stage_population") # Pivot_longer changes data dimensions
